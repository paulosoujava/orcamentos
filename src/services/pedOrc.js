...
exports.create = async function (conn, data, id) {
  ...
  const saveRes = await generic.createOrcPed(data, parseInt(id))
...
exports.update = async function (conn, data, id) {
  const generic = new Generic(conn)
  const result = await generic.checkByIdInOrcPed(id)
...
exports.delete = async function (conn, id) {
  ...
  if (resut.length >= 1) {
    ...
    return _errors(true, 'ops, pedido/orcamento  não encontrado')
  }
}
